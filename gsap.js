...
window.onload = function () {
	...
	gsap.utils.toArray(view.children).forEach((section) => {
		if (section.className !== "mobile-menu") {
	...
	const AboutSection = () => {
		...
		const outlineTimeline = gsap.timeline({
			scrollTrigger: {
				trigger: ".about",
				scrub: 1,
				start: !isMobile ? "left left" : "center center+=350",
	...
	const MobileMenu = () => {
		...
		view.addEventListener("scroll", debounce(handleScroll, 20));
		view.addEventListener("scroll", debounce(handleStickyMenu, 20));
