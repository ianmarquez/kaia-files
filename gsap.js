		...
		gsap.timeline({
			scrollTrigger: {
				...
				onEnter: () => {
					...
					category.classList.add("vision-category-active");
					if (idx > 0) {
						visionCategory[idx - 1].classList.remove("vision-category-active");
					}

					// Animate text change
					if (idx < visionText.length) {
						gsap.to(visionText[idx], {
							opacity: 0,
							y: 40,
							duration: 0.2,
							onComplete: () => {
								visionText.forEach((text, textIdx) => {
									if (textIdx !== idx) {
										text.classList.remove("vision-text-active");
									}
								});

								if (idx + 1 < visionText.length) {
									gsap.to(visionText[idx + 1], {
										opacity: 1,
										y: 0,
										duration: 0.2,
										onStart: () => {
											visionText[idx + 1].classList.add("vision-text-active");
										},
										// onComplete: () => {
										//  gsap.set(visionText[idx + 1], { clearProps: "all" });
										// },
									});
								}
							},
				...
				onLeaveBack: () => {
					// Remove active class from the current category
					category.classList.remove("vision-category-active");
					if (idx > 0) {
						visionCategory[idx - 1].classList.add("vision-category-active");
					}
